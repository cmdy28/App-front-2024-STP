...
const PedidoForm: React.FC<PedidoFormProps> = ({ id, onSuccess }) => {
  ...
  const handleRouteCreated = async (newRouteId: number) => {
    // Actualizar la lista de rutas
    await fetchRutas();
    
    // Buscar la nueva ruta en la lista actualizada
    const newRoute = rutas.find(ruta => ruta.id === newRouteId);
    
    if (newRoute) {
      setFormData({ ...formData, ruta_id: newRouteId });
    ...
    <form onSubmit={handleSubmit}>
      ...
      <Grid item xs={4}>
        <TextField
          ...
          onChange={(e) => {
            handleChange(e);
            const selectedRoute = rutas.find(ruta => ruta.id === Number(e.target.value));
      ...
      {selectedRoute && (
        <Grid item xs={12}>
          <Typography variant="body2">
            Ruta seleccionada: {selectedRoute.ubicacion_origen.nombre} - {selectedRoute.ubicacion_destino.nombre}
          </Typography>
        </Grid>
      )}
        <Grid item xs={4}>
      ...
      >
        <Box sx={{
          ...
          <MapComponent isCreatingFromPedido={true} onRouteCreated={handleRouteCreated} />
        </Box>
      </Modal>
      <br />
      <br />
